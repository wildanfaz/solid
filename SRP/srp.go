...
func NewRepoBook() RepositoryBook {
	return &RepoBook{}
}

func (repo *RepoBook) RepoGetAll() (*Books, error) {
	if len(repo.books) == 0 {
...
func (repo *RepoBook) RepoGetOne(title string) (*Book, error) {
...
func (repo *RepoBook) RepoAdd(book Book) error {
...
func (repo *RepoBook) RepoUpdate(title string, book Book) error {
...
func (repo *RepoBook) RepoDelete(title string) error {
	for i, v := range repo.books {
		if v.Title == title {
